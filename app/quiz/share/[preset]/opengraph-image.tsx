...
export const alt = 'The Nico Starter Questionnaire Result';
...
export default async function Image({ params }: { params: Promise<{ preset: string }> }) {
  ...
  if (!isPresetKey(preset)) {
    return OgImage({ title: 'Nico Questionnaire Result', accent: '#34d399' });
  }
  ...
  return OgImage({
    ...
    subtitle: `Nico Questionnaire result · ${compoundCount} evidence-graded compounds · Tier A/B protocol`,
