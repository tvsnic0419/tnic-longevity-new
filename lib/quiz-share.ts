...
export function buildQuizShareText(answers: QuizAnswers): string {
  ...
  return [
    `My TNiC Nico Starter Questionnaire result: ${result.stack.label}`,
    '',
...
export function buildQuizShareMarkdown(answers: QuizAnswers): string {
  ...
  return `# The Nico Starter Questionnaire Result — ${result.stack.label}
...
- [Retake questionnaire](${quizUrl})

![${result.stack.label} Nico Questionnaire result](${buildQuizOgImageUrl(result.preset as PresetKey)})
