...
export function StackArchitect() {
  ...
  const evidenceCounts = selectedCompounds.reduce<Record<string, number>>((acc, c) => {
    acc[c.evidence] = (acc[c.evidence] ?? 0) + 1;
    return acc;
  }, {});
  const hallmarkCounts = selectedCompounds.reduce<Record<string, number>>((acc, c) => {
    c.hallmarks.forEach((h) => {
      acc[h] = (acc[h] ?? 0) + 1;
    });
    return acc;
  }, {});
  const synergyLinks = selectedCompounds.reduce((count, c) => (
    count + c.synergies.filter((id) => selected.includes(id)).length
  ), 0) / 2;
  const topHallmarks = Object.entries(hallmarkCounts)
    .sort(([, a], [, b]) => b - a)
    .slice(0, 3);
      ...
      <div className="grid lg:grid-cols-12 gap-8">
        ...
        <div className="lg:col-span-5 space-y-6">
          ...
          <div className="glass rounded-2xl p-5">
            <p className="text-[10px] font-mono text-muted-foreground uppercase tracking-wider mb-4">System Readout</p>
            {selected.length === 0 ? (
              ...
              <div className="space-y-4">
                <div className="grid grid-cols-3 gap-2 text-center">
                  <div className="rounded-lg border border-border/60 bg-background/50 p-3">
                    <p className="text-lg font-bold font-mono text-accent-emerald">{evidenceCounts.A ?? 0}</p>
                    <p className="text-[10px] text-muted-foreground">Tier A</p>
                  </div>
                  <div className="rounded-lg border border-border/60 bg-background/50 p-3">
                    <p className="text-lg font-bold font-mono text-accent-cyan">{Object.keys(hallmarkCounts).length}</p>
                    <p className="text-[10px] text-muted-foreground">Hallmarks</p>
                  </div>
                  <div className="rounded-lg border border-border/60 bg-background/50 p-3">
                    <p className="text-lg font-bold font-mono text-accent-violet">{synergyLinks}</p>
                    <p className="text-[10px] text-muted-foreground">Synergy links</p>
                  </div>
                </div>

                <div>
                  <p className="text-xs font-semibold text-foreground mb-2">Most-covered systems</p>
                  <div className="space-y-2">
                    {topHallmarks.map(([hallmark, count]) => (
                      <div key={hallmark}>
                        <div className="flex items-center justify-between text-xs mb-1">
                          <span className="capitalize text-muted-foreground">{hallmark}</span>
                          <span className="font-mono text-accent-cyan">{count}</span>
                        </div>
                        <div className="h-1.5 rounded-full bg-muted overflow-hidden">
                          <div
                            className="h-full rounded-full bg-accent-cyan"
                            style={{ width: `${Math.min(100, (count / Math.max(1, selected.length)) * 100)}%` }}
                          />
                        </div>
                      </div>
                    ))}
