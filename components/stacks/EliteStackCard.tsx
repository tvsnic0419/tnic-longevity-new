...
export function EliteStackCard({ stack, expanded: defaultExpanded = false }: EliteStackCardProps) {
  ...
  return (
    <motion.div layout>
      <GlassPanel depth="mid" className="rounded-2xl">
        <button
          onClick={() => setOpen(!open)}
          className="w-full rounded-2xl text-left p-5 hover:bg-muted/30 transition"
        >
                ...
                {stack.rxCompounds && (
                  <div>
                    ...
                    {stack.rxCompounds.map((rx, i) => (
                      <RevealCard key={rx.id} index={i} depth="mid" className="rounded-xl p-4 border border-accent-rose/20 mb-2">
